...
//            }
        ...
        case .uploadCompositeMultipart(let multipartData, let urlParameters):
            ...
            print("📦 Multipart Data: \(multipartData.count) parts")
            print("📦 URL Parameters:")
            if let urlParameters = urlParameters as? [String: Any],
               JSONSerialization.isValidJSONObject(urlParameters),
               let prettyData = try? JSONSerialization.data(withJSONObject: urlParameters, options: .prettyPrinted),
               let prettyString = String(data: prettyData, encoding: .utf8) {
                print(prettyString)
            } else if let urlParameters = urlParameters as? [Any],
               JSONSerialization.isValidJSONObject(urlParameters),
               let prettyData = try? JSONSerialization.data(withJSONObject: urlParameters, options: .prettyPrinted),
               let prettyString = String(data: prettyData, encoding: .utf8) {
                print(prettyString)
    ...
    /// Pretty prints any JSON-compatible object (dictionary, array, or Encodable type)
    /// - Parameter object: The object to pretty print (can be [String: Any], [Any], or any Encodable type)
    /// - Returns: A pretty-printed JSON string, or a description if not JSON serializable
    func prettyPrintJSON(_ object: Any?)  {
        guard let object = object else {
            print("nil")
            return
        }
        
        // Try to handle as JSON-compatible dictionary
        ...
        // Try to handle as JSON-compatible array
        ...
        // Try to handle as Encodable type using JSONEncoder
        if let encodable = object as? any Encodable,
           let data = try? JSONEncoder().encode(encodable),
           let string = String(data: data, encoding: .utf8) {
            print(string)
        }
        
        // Fallback for non-JSON objects
        print(String(describing: object))
    }

    /// Convenience function that directly prints pretty JSON
    func printPrettyJSON(_ object: Any?, label: String = "") {
        let output: () = prettyPrintJSON(object)
        if !label.isEmpty {
            print("\(label):")
        }
        print(output)
    ...
    public func logResponse(_ response: Response, target: TargetType) {
        ...
        if let headersJSON = try? JSONSerialization.data(withJSONObject: headersDict, options: .prettyPrinted),
           let prettyString = String(data: headersJSON, encoding: .utf8) {
            print(prettyString)
        }
        // Response Data
        print("📥 Response Data:")
    ...
    private func logResponseData(_ data: Data, statusCode: Int) {
        ...
        if let jsonObject = try? JSONSerialization.jsonObject(with: data, options: []),
           let prettyData = try? JSONSerialization.data(withJSONObject: jsonObject, options: .prettyPrinted),  // Changed this line
...
public final class NetworkLoggerPlugin1: PluginType {
